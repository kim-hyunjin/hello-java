    ...
    try {
      ...
      while (true) {
        try {
          ...
          // lesson 객체를 command가 사용하는 객체에 저장한다.
    ...
    try (FileReader in = new FileReader(file); Scanner dataScan = new Scanner(in);) {
      ...
      while (true) {
        try {
          ...
          String[] data = line.split(",");
          Member m = new Member();
          m.setNo(Integer.parseInt(data[0]));
          m.setName(data[1]);
          m.setEmail(data[2]);
          m.setPassword(data[3]);
          m.setPhoto(data[4]);
          m.setTel(data[5]);
          m.setDate(Date.valueOf(data[6]));
          memberList.add(m);
          count++;
        } catch (Exception e) {
          break;
    ...
    try (FileWriter out = new FileWriter(file);) {
      ...
      for (Member m : memberList) {
        String line = String.format("%d,%s,%s,%s,%s,%s,%s\n", m.getNo(), m.getName(), m.getEmail(),
            m.getPassword(), m.getPhoto(), m.getTel(), m.getDate());
        out.write(line);
        count++;
      }
