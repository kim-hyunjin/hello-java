...
public class ApplicationContext03 {
  ...
  public ApplicationContext03(String packageName) throws Exception {
    ...
    File path = Resources.getResourceAsFile(packagePath);// 패키지명을 파일 시스템 경로로 바꿔서 전달한다.
  ...
  private boolean isConcreteClass(Class<?> clazz) {
    if (clazz.isInterface() || clazz.isEnum() || Modifier.isAbstract(clazz.getModifiers())) {
      return false; // 인터페이스, Enum타입, 추상클래스인 경우 객체를 생성할 수 없기 때문에 건너뛴다.
    }
    return true;
  }
  ...
  private void findClass(File path, String packageName) {
    ...
    for (File f : files) {
      ...
      if (f.isFile()) {
        ...
        findClass(f, classOrPackageName);
      }
    }
