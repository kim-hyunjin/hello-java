# 37_3 - Application Server 구조로 변경: Servlet + Dao + 통신규칙2추가
...
### 훈련1: 서버가 클라이언트에게 데이터 입력을 요구할 수 있도록 통신규칙 변경
규칙2) 사용자 입력을 포함하는 경우 
```
[클라이언트]                                        [서버]
서버에 연결 요청        -------------->           연결 승인
...
### 훈련2: '통신규칙2'에 따라 게시물 번호를 입력 받을 수 있도록 BoardDetailServlet을 변경
- com.eomcs.lms.servlet.BoardDetailServlet 변경
  - Servlet 인터페이스에 추가한 service(Scanner in, PrintStream out)을 구현한다.
  - '통신규칙2'에 따라 클라이언트에게 상세 조회할 게시글의 번호를 요구한다.
  - '통신규칙1'에 따라 응답한다.
- com.eomcs.lms.ServerApp 변경
  - '/board/detail' 명령을 처리할 서블릿을 맵에 추가한다.
  
### 훈련3: '통신규칙2'에 따라 게시물 내용을 입력 받을 수 있도록 BoardAddServlet을 변경
...
### 훈련4: '통신규칙2'에 따라 게시물 변경 내용을 입력 받을 수 있도록 BoardUpdateServlet을 변경
...
### 훈련5: '통신규칙2'에 따라 삭제할 게시물 번호를 입력 받을 수 있도록 BoardDeleteServlet을 변경
